...
def paramvals(maxiter, beta, rho, lamb, hess, tau, mbsz, numcon, geomp, stepdecay, gammazero, zeta, N, n, lossbound):
    params = {
        ...
        'lossbound': lossbound # The constraint bound
    }
    return params
...
# Perform the operations in PyTorch
        conf_val = torch.dot(W1, W1) / 4. + torch.dot(W2, W2) / \
            4. + torch.dot(B1, B1) + torch.dot(B2, B2)
        """
        TODO: Compute
    ...
    def conJ(self, params, minibatch):
        ...
        # print(name)
        params = list(self.parameters())
        cgrad = [(params[0]/2.).reshape(-1), 2*params[1].reshape(-1),
                 (params[2] / 2.).reshape(-1), 2*params[3].reshape(-1)]
...
for trial in range(trials):  # X_train = torch.tensor()
    ...
    params = paramvals(maxiter=maxiter, beta=10, rho=1., lamb=0.5, hess='diag', tau=1., mbsz=100,
                       numcon=1, geomp=0.5, stepdecay='dimin', gammazero=0.1, zeta=0.1, N=X_train.shape[0], n=num_param, lossbound=[2.5])
