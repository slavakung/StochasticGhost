...
for trial in range(trials):
    ...
    X_train, X_val, Y_train, Y_val = train_test_split(
        torch.tensor(xd.values), torch.tensor(y), test_size=0.4, random_state=42)
    train_loss_list = []
    val_loss_list = []
    ...
    params = paramvals(maxiter=maxiter, beta=10., rho=0.8, lamb=0.5, hess='diag', tau=1., mbsz=10,
...
plt.title("Validation loss for MNIST label 2")
...
# plt.plot(range(0, maxiter+1), convals[:, 0], 'k-')
# plt.fill_between(range(0, maxiter+1), convals[:, 0], convals[:, 1])
plt.title("Training loss for MNIST label 2")
plt.xlabel('Iteration')
plt.ylabel('Loss')
